...
- **自動分房**：每頻道獨立分房，每房上限 200 人（可調），滿了自動開新房；彈幕只在房內廣播，控制伺服器頻寬成本
- **防洗版**：同 IP 節流（每 1.5 秒才能再發，前端按鈕同步冷卻、後端依 IP 把關，擋多分頁繞過）＋ 令牌桶限流（可連發 3 則，之後每 2 秒補一則額度），單則最長 50 字
